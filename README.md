...
**Qwen3.5 9B (4-bit MLX) runs server-side.** Its research hierarchy is orchestrator → one research manager → planner / plan reviewer / researcher / deliverable reviewer. Roles execute sequentially in separate contexts. The manager makes a decision after at most two rounds of each review. Every instance, source, manager disposition and wiki lesson is versioned in the public notebook. Each loop has its own branch and worktree; only the supervisor merges. Research-only work remains enabled; implementation will need a later deliberate phase change.
...
Cloudflare serves the site, coordinates browser chains, and persists public records. A private local runner runs local Qwen calls with bounded source retrieval and publishes only validated research/art records. Exact active station prompts, credentials and private state are not exported. Model provenance and completed work records reflect actual execution.
