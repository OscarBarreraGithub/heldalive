...
if (!["localhost", "127.0.0.1"].includes(new URL(base).hostname))
  throw new Error("Integration fixtures must only run locally.");
const opened = [];
async function peer(room = "browser") {
  const url = new URL(`/api/socket?room=${room}`, base);
  url.protocol = "ws:";
...
async function rejected(query, origin, expected) {
  const url = new URL(`/api/socket?${query}`, base);
  url.protocol = "ws:";
...
try {
  ...
  const jobEvent = await until(
    () => contributor.events.find((e) => e.type === "job"),
    35000,
  );
  assert.ok(
    jobEvent.job.messages.some((m) =>
      m.content.includes("consenting visitors"),
    ),
  );
  ...
  contributor.send({
    ...
    text: "Integration fixture: a thought from its assigned contributor.",
  });
  contributor.send({ type: "done", jobId: jobEvent.job.id, tokens: 12 });
  const completed = await until(() =>
    watcher.events.find(
      (e) =>
        e.type === "state" && e.thoughts.some((t) => t.id === jobEvent.job.id),
    ),
  );
  assert.equal(
    completed.thoughts.at(-1).text,
    "Integration fixture: a thought from its assigned contributor.",
  );
  assert.equal(completed.thoughts.at(-1).tokens, 12);
  assert.equal(completed.active, null);
  console.log(
    "PASS: real job assignment, owner-only output, shared completion, persisted counters",
  );
  contributor.send({ type: "ready", ready: false });
  const withdrawal = await until(() =>
    watcher.events
      .slice()
      .reverse()
      .find(
        (e) =>
          e.type === "state" && e.contributors === 0 && e.phase === "waiting",
      ),
  );
  assert.equal(withdrawal.modelAvailable, false);
  console.log(
    "PASS: withdrawal stops browser inference without a Mac fallback",
  );
  ...
  await until(() =>
    watcher.events.some(
      (e) => e.type === "error" && e.message.includes("Give this thought"),
    ),
  );
  console.log("PASS: note acceptance and rate limiting");
  const state = await fetch(`${base}/api/state?room=browser`).then((r) =>
    r.json(),
  );
  ...
  console.log(
    "PASS: state readback excludes connection identity and private scheduling data",
  );
} finally {
