...
export async function loadTokenizer() {
  const response = await fetch(WEIGHT_BASE + "tokenizer.json");
  if (!response.ok) throw Error("Tokenizer unavailable");
  return createByteLevelTokenizer(await response.json());
}
export async function loadStage(
  ...
  const manifest = (await response.json()) as {
    ...
    buffers: Record<string, { bytes: number; sha256: string }>;
  };
  if (
  ...
  const kv = Array.from({ length: 32 }, (_, i) =>
    device.createBuffer({
      size:
