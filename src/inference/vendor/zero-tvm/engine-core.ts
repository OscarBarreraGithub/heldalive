...
export interface DecodeEngine {
  ...
  pipelineStep(
    ...
    position: number,
  ): Promise<{ residual: ArrayBuffer } | { tokenId: number; logits: Float32Array }>
  pipelineBatch(input: {tokens:number[]} | {residuals:ArrayBuffer}, position:number, count:number): Promise<{residuals:ArrayBuffer}|{logits:Float32Array}>
...
export function buildDecodeEngine(
  ...
  const D_WGS   = Math.ceil(S.d / WG_SIZE_D)             // Phi-3: 12   (3072/256)
  const QKV_WGS = Math.ceil(S.qkvDim / WG_SIZE_D)        // Phi-3: 36   (9216/256)
  const KV_WGS  = Math.ceil(S.kvDim / WG_SIZE_D)         // kv_append grid — Phi-3: 12, Qwen3: 4
  ...
  async function pipelineStep(
    ...
    if (last) {
      ...
      readBuf.unmap()
      if (!logitsReadBuf) logitsReadBuf = device.createBuffer({size:S.vocab*4, usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST, label:'pipelineLogits'});
      const read = device.createCommandEncoder();
      read.copyBufferToBuffer(B.logits, 0, logitsReadBuf, 0, S.vocab*4);
  ...
  // Held Alive: amortize CPU/GPU readback over a bounded input batch. Queue writes
  // and submissions remain ordered per token, so every token sees its own position.
  // Each stage still applies exactly its layer range; only readback is batched.
  async function pipelineBatch(input:{tokens:number[]}|{residuals:ArrayBuffer},position:number,count:number):Promise<{residuals:ArrayBuffer}|{logits:Float32Array}>{
    if(!Number.isInteger(count)||count<1||count>16||position<0||position+count>MAX_CONTEXT)throw Error('Invalid pipeline batch');
    if(('tokens'in input)!==(L0===0))throw Error('Wrong pipeline input');
    if('tokens'in input?input.tokens.length!==count:input.residuals.byteLength!==count*S.d*2)throw Error('Wrong batch shape');
    if(pooling)throw Error('Batched pipeline is limited to resident dense models');
    const last=L1===S.layers;
    const read=device.createBuffer({size:last?S.vocab*4:count*S.d*2,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST,label:'held-batch-readback'});
    try{
      for(let i=0;i<count;i++){
        if('tokens'in input)writeStepState(input.tokens[i],position+i);
        else{device.queue.writeBuffer(B.residual,0,input.residuals,i*S.d*2,S.d*2);writeStepState(null,position+i);}
        const enc=device.createCommandEncoder();recordPrologue(enc);
        for(let L=L0;L<L1;L++){const blk=layerBGs[L-L0];recordAttn(enc,blk,position+i);recordFfnTail(enc,L,blk);}
        if(last){if(i===count-1){recordEpilogue(enc);enc.copyBufferToBuffer(B.logits,0,read,0,S.vocab*4);}}
        else enc.copyBufferToBuffer(B.residual,0,read,i*S.d*2,S.d*2);
        device.queue.submit([enc.finish()]);
      }
      gdnStatePos=position+count;
      await read.mapAsync(GPUMapMode.READ);const out=read.getMappedRange().slice(0);read.unmap();
      return last?{logits:new Float32Array(out)}:{residuals:out};
  ...
  return {
    ...
    pipelineBatch: guard('pipelineBatch', pipelineBatch),
